...
- [~] **Voice / LiveKit UI tests** — partial: pre-join in [`voice-channel.spec.ts`](apps/web/e2e/voice-channel.spec.ts) (2 tests); full create+join flow in [`community.spec.ts`](apps/web/e2e/community.spec.ts) un-fixme'd and passing now that the cold-context bug is fixed. Still untested: `VoiceSettingsModal` device switching, focus mode, PiP, reconnect. Those need separate test passes against a now-functional Join Voice path.
...
1. **Deferred Voice/LiveKit coverage** — VoiceSettingsModal device switching, focus mode, PiP, reconnect. Now unblocked.
