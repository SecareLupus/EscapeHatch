...
- [~] **Voice / LiveKit UI tests** — partial: added [`voice-channel.spec.ts`](apps/web/e2e/voice-channel.spec.ts) (2 tests) covering pre-join surface (channel creation, channel routing, Join Voice button + debug-state markers). Post-join surface (`VoiceRoom`, `VoiceSettingsModal`, device switching, focus mode, PiP, reconnect) **blocked by Phase 26 cold-context bug** — clicking Join Voice from a fresh page redirects back to home hub. Resume coverage after that bug ships.
...
1. **Voice join cold-context bug (Phase 26)** — fix the `test.fixme` in `community.spec.ts` so `data-voice-connected=true` doesn't immediately flip back and the page doesn't redirect to home hub. **Unblocks the deferred half of Voice/LiveKit UI tests** (`VoiceSettingsModal`, device switching, focus mode, PiP, reconnect). Highest-priority next item.
